...
class InferenceCore:
    def __init__(self, prop_net:STCN, images, flows, num_objects, top_k=20, mem_every=5, include_last=False):
        ...
        flows, self.pad = pad_divide_by(flows, 16)
    ...
    def encode_flow(self, idx):
        result = self.prop_net.encode_flow(self.flows[:,idx].cuda())
        return result
    ...
    def do_pass(self, key_k, key_v, idx, end_idx):
        ...
        for ti in this_range:
            ...
            s16, s8 , s4 = self.encode_flow(ti)
            out_mask = self.prop_net.segment_with_query(self.mem_bank, qf8, qf4, k16, qv16, s16, s8, s4)
