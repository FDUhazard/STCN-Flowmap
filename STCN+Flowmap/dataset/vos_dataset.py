...
class VOSDataset(Dataset):
    ...
    """
    def  __init__(self, im_root, gt_root, flow_root, max_jump, is_bl, subset=None):
    ...
    def __getitem__(self, idx):
        ...
        vid_flow_path = path.join(self.flow_root, video)
        ...
        while trials < 5:
            ...
            frames_idx = [start_idx, f1_idx, f2_idx] #随机选择3帧
            ...
            for f_idx in frames_idx:
                ...
                flow_name = frames[f_idx][:-4] + '.jpg'  # 格式是否为jpg？
                ...
                reseed(sequence_seed)
                this_flow = Image.open(path.join(vid_flow_path, flow_name)).convert('RGB')
                this_flow = self.all_im_dual_transform(this_flow)
                this_flow = self.all_im_lone_transform(this_flow)
                ...
                reseed(pairwise_seed)
                this_flow = self.pair_im_dual_transform(this_flow)
                this_flow = self.pair_im_lone_transform(this_flow)

                this_im = self.final_im_transform(this_im)
                this_gt = np.array(this_gt)
                this_flow = self.final_im_transform(this_flow)
            ...
            labels = np.unique(masks[0]) # np.unique找mask中存在的object类有哪些
            # Remove background
