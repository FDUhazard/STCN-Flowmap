...
class ResBlock(nn.Module):
    def __init__(self, indim, outdim=None):  # 1024->512
...
class FeatureFusionBlock(nn.Module):
    def __init__(self, indim, outdim):
        ...
        self.attention = cbam.CBAM(outdim)  # 注意力机制相关？需学习
...
class UpsampleBlock(nn.Module):  # skip-connection?上采样
...
class KeyProjection(nn.Module):  # 用于何处?
...
class FlowProjection(nn.Module): 
    def __init__(self, indim, keydim):
        super().__init__()
        self.flow_proj = nn.Conv2d(indim, keydim, kernel_size=3, padding=1)

        nn.init.orthogonal_(self.flow_proj.weight.data)
        nn.init.zeros_(self.flow_proj.bias.data)
    
    def forward(self, x):
        return self.flow_proj(x)
