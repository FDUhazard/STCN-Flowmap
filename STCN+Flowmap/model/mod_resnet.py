...
def load_weights_sequential(target, source_state, extra_chan=1):  # 什么作用？
...
class Bottleneck(nn.Module):  # 1*1 -> 3*3 -> 1*1通道4倍
    expansion = 4
...
class ResNet(nn.Module):
    def __init__(self, block, layers=(3, 4, 23, 3), extra_chan=1):  # layers为每一层layer中block的数量blocks
        self.inplanes = 64
